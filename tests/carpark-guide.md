...
| ---------- |-----------------------------------------------------|----------------------------------------------|
| `CarPark`    | location, capacity,<br/>plates, sensors<br/>diplays | add_car, remove car<br/>update display       |
| `Sensor`     | id, is_active, car_park                             | update_car_park, _scan_plate, detect_vehicle |
| `Display`    | id, message, is_on, car_park                        | __init__, __str__, update                    |
...
>Class CarPark manages the count with respect to the number of bays available.
> Class CarPark manages the overall number of cars by having overall capacity and list of current plates (cars inside the park). 
> The available bays are obtained as a subtraction of total capacity from the number of automobiles occupied bays. 
> Hence, the CarPark class has every reason to be the one who controls this kind of information.
>Q. Which class is responsible for the current temperature (and why)?
>The current temperature could be responsible with the CarPark class.
> If there is such a system present in the car park for monitoring the weather conditions, for example, the temperature, 
> this feature can be included in the CarPark class. But If the temperature is taken through a specific measure such as 
> the temperature measure used in the car park, then a different temperature sensor class will be used.
>Q. Which class is responsible for the time (and why)?
>The CarPark class or Sensor class
> Generally, the entry or exit time of the cars from the car parking is mentioned. However, 
> as the CarPark class maintains records of its activities (entry & exit), it is logical to 
> allow the CarPark class to maintain current time while logging vehicle activity events. 
> The same time information could be collected from the Sensor class as well, if it is constructed 
> to contain a vehicle entry and exit detection feature. As to the issue of recording the time of car activities, 
> the CarPark class can be said to record the time since it has control over the whole car park system.
--------
...
> Given that the register method is a member of this class CarPark, it is necessary to incorporate it within the unit tests since the method manages 
> the registration of Sensor and Display objects. The tests ought to establish that the method accepts appropriate Sensor and Display objects in the car 
> park while TypeError is raised on an invalid object registration. In doing so, it makes sure that the CarPark class contains it components in a proper 
> manner which doesnô t affect the car park system.
